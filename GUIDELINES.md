# MicroLoss Tutorial — Guidelines & Style Guide

These guidelines capture the principles, patterns, and lessons learned from writing and refining the tutorial. Use them when writing new sections or reviewing existing ones. They supplement the rules in `CLAUDE.md` with specific, actionable guidance.
...
## 3. Visualisations & Interactive Elements
...
- Each interactive visualisation must be wholly self-contained (CLAUDE.md rule 11). It should not depend on state from a previous section.
...
- Use callout boxes (styled `div` with `rounded-lg border border-surface-lighter bg-surface-light/50 p-4 text-sm text-text-secondary`) for:
  - Notation explanations (e.g., "A note on 'log'")
  - Historical context (e.g., "Why 'cross-entropy'?")
  - Tangential but interesting connections
- Callout boxes should have a **bold title** as the first element: `<strong class="text-text-primary">Title:</strong>`
- Information in callout boxes must stand alone — the reader should be able to skip it without losing the thread.
...
- Never say "X is Y" when X is encoded/represented as Y (CLAUDE.md rule 15).
...
## 7. Checklist for Reviewing a Section
